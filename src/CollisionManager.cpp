...
bool CollisionManager::squaredRadiusCheck(Player* player, GameObject* object2)
{
	...
	if (object2->getPosition().x <= -60)// && object2->getPosition().x >= -64.5f)
	{
		TheGameManager::Instance()->addBirdsAvoided();
		object2->setPosition(8000, -500);
		std::cout << "Bird Avoided" << std::endl;
	}
			...
			case ENEMY:
				{
					...
					TheGameManager::Instance()->decreaseScore(200);
					TheGameManager::Instance()->addBirdsDestroyed();
					explosion->activate();
					explosion->setPosition(glm::vec2(object2->getPosition()));
				}
		
				//TheGameManager::Instance()->setPlayerHealth(TheGameManager::Instance()->getPlayerLife() - 1);
